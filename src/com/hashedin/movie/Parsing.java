 ...
 * 
 * @author sourabh
 * this class is used for parsing the file 
 */
...
public class Parsing {
	...
	public Map<Integer, Movie> getMovieData() {
		...
		Movie movie1 = new Movie(1, "Silence of lambs ", genreArray1,
				"12 dec 2012");
		Movie movie2 = new Movie(2, "Pay it forward", genreArray2,
				"12 dec 2003");
		Movie movie3 = new Movie(3, "The Green Mile", genreArray3,
				"12 dec 2002");
		...
		/*
		 * //parsing of file Map<Integer, Movie> movieData = new
		 * HashMap<Integer, Movie>();
		 * 
		 * Movie movie1 = new Movie(); try {
		 * 
		 ...
		 * 
		 * String inputLine = in.readLine(); String tokens[] =
		 * inputLine.split("[|]+");
		 * 
		 * movie1.setMovieId(Integer.valueOf(tokens[0]));
		 * movie1.setMovieName(tokens[1]); movie1.setDate(tokens[2]);
		 * 
		 * //genreArray for the list of specified genre of a movie
		 * 
		 * int genreArray[] = new int[19];
		 * 
		 * for (int t = 0; t < genreArray.length; t++) { genreArray[t] =
		 * Integer.valueOf(tokens[4 + t]); }
		 * 
		 * movie1.setGenre(genreArray); //System.out.println(movie1);
		 * movieData.put(movie1.getMovieId(), movie1);
		 * 
		 * } in.close(); } catch (Exception e) {
		 * System.err.println("input file does not exists"); } return movieData;
		 * 
		 */
	...
	public List<Rating> getRating() {
		...
		/*
		 * List<Rating> userRating = new ArrayList<Rating>(); Rating ratingObj =
		 * new Rating();
		 * 
		 * try {
		 * 
		 * //Input rating.data file FileInputStream fstream = new
		 * FileInputStream("ratingsEdit.data");
		 * 
		 * 
		 * DataInputStream in = new DataInputStream(fstream);
		 * 
		 * while (in.available() != 0) {
		 * 
		 * //split the rating.data file into tokens String inputLine =
		 * in.readLine();
		 * 
		 ...
		 * 
		 * } in.close(); } catch (Exception e) {
		 * System.err.println("input file does not exists"); } return
		 * userRating;
		 */
	}
