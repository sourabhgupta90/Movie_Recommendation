...
/**
 * @author sourabh 
 * contains the assignment problem 
 */
...
public class MovieBuff {
	...
	/**
	 * 
	 * @return the most watched movie name
	 */
	...
	public String getMostWatchedMovieName() {
		...
		for (int t = 0; t < ratingListObj.size(); t++) {
			...
			// ratingListObj.get(t).getMovieObj().getMovieId());
	...
	/**
	 * 
	 * @return the movie Id list of specified genre
	 */
	public List<Integer> getMovieIdListByGenre() {
		...
		for (Rating ratingObj : ratingListObj) {
			...
			if (ratingObj.getMovieObj().getGenreobj()
					.contains(g.fromCode(genreInput))) {
	...
	/**
	 * 
	 * @return the Movie Id list By Year
	 */
	...
	public List<Integer> getMovieIdListByyear() {
		...
		for (Rating ratingObj : ratingListObj) {
			...
			String year = ratingObj.getMovieObj().getDate()
					.substring(stringLength - 4, stringLength);
	 ...
	 * 
	 * @param movieIdList
	 *            is the list of movie Id
	 * @return the top movie rating of specified list
	 */
	...
	/**
	 * 
	 * @param movieIdListByGenre
	 *            is the list of movie of specified genre
	 * @param movieIdListByYear
	 *            is the list of movie of specified year
	 * @return return the movie id which comes in both specified movie
	 */
	public List<Integer> getMovieIdListByYearAndGenre(
			List<Integer> movieIdListByGenre, List<Integer> movieIdListByYear) {
	...
	/**
	 * 
	 * @return the most watched user id
	 */
	public int mostWatchedUserId() {
		...
		for (Rating ratingObj : ratingListObj) {
			...
			// ratingObj.getUserObj().getUserId());
